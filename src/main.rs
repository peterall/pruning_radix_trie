...
impl PruningRadixTrie {
    ...
    fn add_top_k_result(&self, term: &str, weight: u32, top_k: usize, results: &mut Vec<(String, u32)>) {
    ...
    fn find_all_child_terms(&self, node: &Node, prefix: &str, matched_prefix: &mut String, top_k: usize, results: &mut Vec<(String, u32)>) {
        if let Some(children) = &node.children {
            ...
            for (term, child_id) in children {
                ...
                if prefix.len() == 0 || term.starts_with(prefix) {
                    ...
                    matched_prefix.push_str(term);
                    self.find_all_child_terms(child, &prefix[term.len()..], matched_prefix, top_k, results);
                    matched_prefix.truncate(matched_prefix.len() - term.len());
                }
            }
        }
        ...
        match self.match_children(curr_id, &term) {
            ...
            NodeMatch::CommonSubstring(NodeMatchContext {
                ...
                let child_max_weight =
                    max(node.child_max_weight, max(weight, node.weight));
...
fn main() {
    ...
    println!("{:-^50}", " Pruning Radix Trie ");
    // trie.add("hello".to_owned(), 12);
    // trie.add("he".to_owned(), 15);
    // trie.add("helloworld".to_owned(), 5);
    // trie.add("hellowood".to_owned(), 20);
    // trie.add("hella".to_owned(), 30);
    // trie.add("helli".to_owned(), 21);
    // trie.add("hellia".to_owned(), 10);
    load_terms(&mut trie);
    ...
    println!("Done");
    // println!("here's what I found: {:?}", keys);
}
